...
| 模块 | 触发方式 | 主要流程 | 结果 |
|------|----------|----------|------|
| 签到 | 每日定时任务，默认 01:10 UTC / 北京时间 09:10 | 读取 Cookie -> 判断是否已签到 -> 未签到则领取每日奖励并查询余额 | 写回续期 Cookie，失败时推送告警 |
| 保活 | 每 6 小时定时任务 | 带 Cookie 访问 V2EX 首页 | 触发滑动续期并写回新 Cookie |
| 自动阅读 | 每日定时任务，默认 01:15 UTC / 北京时间 09:15 | 启动 Playwright -> 注入指纹和 Cookie -> 从队列取帖 -> 随机停留与滚动阅读 -> 定期检查余额 | 余额变化达标、篇数达上限、超时或确认登录失效时安全退出 |
| Telegram Bot | 常驻长轮询 | 首次私聊绑定授权用户；响应 `/start`、`/sou`、`/tasks`、`/checkin`、`/read`、`/cookie`、`/debug`、`/stop` 等命令 | 远程查看状态、导入 Cookie、手动触发或停止任务 |
