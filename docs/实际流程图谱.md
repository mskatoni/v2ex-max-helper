...
本文依据当前本地 `mskatoni-patch-beta` 工作树中的代码绘制，用来回答“系统现在实际上怎样运行”。对应目标见 [设计流程图谱.md](设计流程图谱.md)。当前 Git HEAD 为 `6560894`（标签 `v1.3.6`），下述 `v1.3.7` 修复仍只保留在本地工作树。
...
flowchart TD
    ...
    D1 --> H0["后台启动 server.js 健康检查"]
    H0 --> B0["后台启动 reader/bot.js"]
    B0 --> C0["lib/config.js 加载配置并安装代理"]
    C0 --> P0{"代理配置是否有效"}
    P0 -->|"有效或未启用"| A0["加载 Telegram 授权和 profile 列表"]
    P0 -->|"无效"| X0["模块加载异常并退出"]
    A0 --> O0["确认 Telegram getUpdates 唯一所有权"]
...
Docker Compose 当前使用 `TZ=${TZ:-Asia/Shanghai}`，因此 Node.js 的本机时间由容器 `TZ` 决定。Docker CLI 在本机不可用，镜像构建和真实容器启动尚未验证。
...
flowchart TD
    ...
    A0 -->|"否"| X0["忽略"]
    A0 -->|"是"| C0{"命令或按钮"}
    C0 -->|"签到"| P0["多于一个 profile 时选择下标"]
    C0 -->|"阅读"| P1["选择 profile 后选择篇数"]
    C0 -->|"余额"| P2["读取 profile 余额文件"]
    C0 -->|"Cookie"| P3{"先选 profile 还是先粘贴"}
    P0 --> C1["检查 Cookie 和任务互斥后启动"]
    P1 --> R0["检查 Cookie 和全局阅读锁后启动"]
    P3 -->|"先选 profile"| K0["等待用户粘贴 Cookie"]
    P3 -->|"先粘贴"| K1["随机 nonce 暂存完整候选字段"]
    K0 --> K2["请求认证页确认真实账号"]
...
文本命令支持 `/sou [profile]`、`/checkin [profile]`、`/read [数量]`、`/read profile [数量]`、`/stop [profile]` 和 `/cookie [profile] [内容]`。回调使用短期会话 nonce 与下标；旧版无账号阅读按钮只返回失效提示。
...
| 账号身份 | `profile_identity.json` | `profile_identity.<profile>.json` | 加盐哈希隔离 |
...
| 单账号调度 | 本机 09:10 签到，09:15 阅读 | 调度器按 `Date.getHours()` 和 `getMinutes()` 执行 | 一致 |
...
| 版本发布状态 | 本地代码升至 1.3.7，但不提交和发布 | tracked 文件为 1.3.7，Git HEAD 和 tag 仍为 1.3.6；忽略的 lockfile 仅本地更新 | 符合本地修改约束 |
