...
function isDockerHostAlias(hostname) {
  return DOCKER_HOST_ALIASES.has(String(hostname || '').replace(/^\[|\]$/g, '').toLowerCase());
}
...
function shouldBypassProxy(hostname, port) {
  const targetHost = String(hostname || '').replace(/^\[|\]$/g, '').replace(/\.$/, '').toLowerCase();
